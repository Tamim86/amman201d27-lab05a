...
function sumAndMultiply(a, b, c){
  let newsum = sum(a,b);
  let sum2= sum(newsum[0],c)[0];
  let newmult= multiply(a,b);
  let mult2= multiply(newmult[0],c)[0];
...
function sumArray(sumArr) {
  //eslint-disable-line
  sum = 0;
  for(let v=0; v<testArray.length; v++){
    sum = sum+testArray[v];
...
function multiplyAnyArray(dynamicArray) {
  //eslint-disable-line
  let c= 1;
  for(let i=1;i<testDynamicArray.length;i++){
    c*=testDynamicArray[i];
  }
  return[c, 'The numbers '+testDynamicArray+' have a product of '+c+'.']
}
